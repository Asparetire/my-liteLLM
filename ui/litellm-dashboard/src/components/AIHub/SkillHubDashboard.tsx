...
function SkillsEmptyState({ filtered, t }: { filtered: boolean; t: Translator }) {
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      <div className="mb-1 flex size-10 items-center justify-center rounded-lg bg-muted">
        <Inbox className="size-5 text-muted-foreground" />
      </div>
      <div className="text-sm font-medium text-foreground">{filtered ? t("noMatchingSkills") : t("noSkillsYet")}</div>
      <div className="text-sm text-muted-foreground">
        {filtered ? t("adjustSkillsFilterHint") : t("skillsAppearHint")}
      </div>
    </div>
  );
}
...
const SkillHubDashboard: React.FC<SkillHubDashboardProps> = ({
  ...
  const domainItems = useMemo(
    () => [{ value: ALL_DOMAINS, label: t("allDomains") }, ...domains.map((d) => ({ value: d, label: d }))],
    [domains, t],
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="border border-border rounded-lg p-4">
          <div className="text-xs text-muted-foreground mb-1">{t("totalSkillsStat")}</div>
          <div className="text-2xl font-semibold text-foreground">{totalSkills}</div>
        </div>
        <div className="border border-border rounded-lg p-4">
          <div className="text-xs text-muted-foreground mb-1">{t("namespacesStat")}</div>
          <div className="text-2xl font-semibold text-foreground">{namespaces.length}</div>
        </div>
        <div className="border border-border rounded-lg p-4">
          <div className="text-xs text-muted-foreground mb-1">{t("domainsStat")}</div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-foreground">
            {publicPage ? t("allPublicSkillsTitle") : t("allSkillsTitle")}
          </h3>
        ...
        <DataTable
          ...
          isLoading={isLoading}
          loadingMessage={t("loadingSkills")}
          noDataMessage={<SkillsEmptyState filtered={hasActiveFilter} t={t} />}
          size="compact"
        />
        <div className="mt-3 text-center">
          <p className="text-sm text-muted-foreground">{t("showingSkillsOf", { shown: filteredSkills.length, total: totalSkills })}</p>
        </div>
      </div>
    </div>
  );
