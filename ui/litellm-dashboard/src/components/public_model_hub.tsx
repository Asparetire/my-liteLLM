...
import { capabilityLabel } from "./AIHub/ModelHubTableColumns";
import {
  AgentCard,
  MCPServerData,
  ModelGroupInfo,
...
const PublicModelHub: React.FC<PublicModelHubProps> = ({ accessToken, isEmbedded = false }) => {
  ...
  const serviceStatus = models.error ? t("serviceUnavailable") : t("serviceAvailable");
  ...
  const agentColumns = useMemo(() => getPublicAgentHubColumns({ onAgentClick: showAgentModal, t }), [showAgentModal, t]);
        ...
        <div className={isEmbedded ? "w-full" : "min-h-screen bg-card"}>
          ...
          <div className={isEmbedded ? "w-full p-6" : "w-full px-8 py-12"}>
            ...
            {isEmbedded && (
              <div className="mb-6 p-4 bg-info/10 border border-info/20 rounded-lg">
                <p className="text-sm text-foreground">{t("embeddedHubDescription")}</p>
              </div>
            )}
            ...
            {!isEmbedded && (
              <Card className="mb-10 p-8 bg-card border border-border rounded-lg shadow-xs">
                <h2 className="text-2xl font-semibold mb-6 text-foreground">{t("aboutTitle")}</h2>
                <p className="text-foreground mb-6 text-base leading-relaxed">
                  {customDocsDescription ? customDocsDescription : t("defaultHubDescription")}
                </p>
                <div className="flex items-center space-x-3 text-sm text-muted-foreground">
                  <span className="flex items-center">
                    <span className="w-4 h-4 mr-2">🔧</span>
                    {t("builtWithLitellm", { version: litellmVersion })}
                  </span>
                </div>
              </Card>
            )}
            ...
            {usefulLinks && Object.keys(usefulLinks).length > 0 && (
              <Card className="mb-10 p-8 bg-card border border-border rounded-lg shadow-xs">
                <h2 className="text-2xl font-semibold mb-6 text-foreground">{t("usefulLinksTitle")}</h2>
            ...
            {!isEmbedded && (
              <Card className="mb-10 p-8 bg-card border border-border rounded-lg shadow-xs">
                <h2 className="text-2xl font-semibold mb-6 text-foreground">{t("healthStatusTitle")}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <p className="text-success font-medium text-sm">{t("serviceStatusLabel", { status: serviceStatus })}</p>
                </div>
              </Card>
            )}
            ...
            <Card className="p-8 bg-card border border-border rounded-lg shadow-xs">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="public-hub-tabs">
                <TabsList>
                  <TabsTrigger value="models">{t("tabModelHub")}</TabsTrigger>
                  {hasAgents && <TabsTrigger value="agents">{t("tabAgentHub")}</TabsTrigger>}
                  {hasMcpServers && <TabsTrigger value="mcp">{t("tabMcpHub")}</TabsTrigger>}
                  <TabsTrigger value="skills">{t("tabSkillHub")}</TabsTrigger>
                </TabsList>

                {/* Models Tab */}
                <TabsContent value="models">
                  <div className="flex justify-between items-center mb-8">
                    <h2 className="text-2xl font-semibold text-foreground">{t("availableModelsTitle")}</h2>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 p-6 bg-muted rounded-lg border border-border">
                    <div>
                      <div className="flex items-center space-x-2 mb-3">
                        <p className="text-sm font-medium text-foreground">{t("searchModelsLabel")}</p>
                        <Tooltip>
                          <TooltipTrigger render={<Info className="w-4 h-4 text-muted-foreground cursor-help" />} />
                          <TooltipContent side="top">{t("searchModelsTooltip")}</TooltipContent>
                        </Tooltip>
                      </div>
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium mb-3 text-foreground">{t("providersLabel")}</p>
                      <Combobox
                        multiple
                      ...
                      >
                        <ComboboxChips render={<div ref={anchor} />} className="min-h-8 w-full py-1 text-sm">
                          ...
                          <ComboboxChipsInput
                            ...
                            aria-label={t("selectProviders")}
                            className="min-w-24"
                          />
                        </ComboboxChips>
                        <ComboboxContent anchor={anchor}>
                          <ComboboxEmpty>{t("noProvidersFound")}</ComboboxEmpty>
                          <ComboboxList>
                            {(provider: string) => {
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium mb-3 text-foreground">{t("modeLabel")}</p>
                      <MultiSelect
                        options={modeOptions}
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium mb-3 text-foreground">{t("featuresLabel")}</p>
                      <MultiSelect
                  ...
                  <DataTable
                    ...
                    noDataMessage={
                      <PublicHubEmptyState
                        title={models.hasActiveQuery ? t("noMatchingModels") : t("noModelsYet")}
                        body={
                          models.hasActiveQuery
                            ? t("publicAdjustModelsHint")
                            : t("publicModelsAppearHint")
                        }
                      />
                    }
                    size="compact"
                ...
                {hasAgents && (
                  <TabsContent value="agents">
                    <div className="flex justify-between items-center mb-8">
                      <h2 className="text-2xl font-semibold text-foreground">{t("availableAgentsTitle")}</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 p-6 bg-muted rounded-lg border border-border">
                      <div>
                        <div className="flex items-center space-x-2 mb-3">
                          <p className="text-sm font-medium text-foreground">{t("searchAgentsLabel")}</p>
                          <Tooltip>
                            <TooltipTrigger render={<Info className="w-4 h-4 text-muted-foreground cursor-help" />} />
                            <TooltipContent side="top">{t("searchAgentsTooltip")}</TooltipContent>
                          </Tooltip>
                        </div>
                        <div className="relative">
                          <SearchIcon className="w-4 h-4 text-muted-foreground absolute left-3 top-1/2 transform -translate-y-1/2" />
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium mb-3 text-foreground">{t("skillsLabel")}</p>
                        <MultiSelect
                    ...
                    <DataTable
                      ...
                      loadingMessage={t("loadingAgents")}
                      noDataMessage={
                        <PublicHubEmptyState title={t("noMatchingAgents")} body={t("publicAdjustAgentsHint")} />
                      }
                      size="compact"
                    />
                    ...
                    <div className="mt-8 text-center">
                      <p className="text-sm text-muted-foreground">
                        {t("showingAgentsOf", { total: agentHubData?.length || 0, shown: filteredAgentData.length })}
                      </p>
                    </div>
                  </TabsContent>
                )}
                ...
                {hasMcpServers && (
                  <TabsContent value="mcp">
                    <div className="flex justify-between items-center mb-8">
                      <h2 className="text-2xl font-semibold text-foreground">{t("availableMcpServersTitle")}</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 p-6 bg-muted rounded-lg border border-border">
                      <div>
                        <div className="flex items-center space-x-2 mb-3">
                          <p className="text-sm font-medium text-foreground">{t("searchMcpLabel")}</p>
                          <Tooltip>
                            <TooltipTrigger render={<Info className="w-4 h-4 text-muted-foreground cursor-help" />} />
                            <TooltipContent side="top">{t("searchMcpTooltip")}</TooltipContent>
                          </Tooltip>
                        </div>
                        <div className="relative">
                          <SearchIcon className="w-4 h-4 text-muted-foreground absolute left-3 top-1/2 transform -translate-y-1/2" />
                          <input
                            type="text"
                            placeholder={t("searchMcpPlaceholder")}
                            value={mcpSearchTerm}
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium mb-3 text-foreground">{t("transportLabel")}</p>
                        <MultiSelect
                          options={mcpTransportOptions}
                          value={selectedMcpTransports}
                          onValueChange={setSelectedMcpTransports}
                          placeholder={t("selectTransports")}
                          className="w-full"
                        />
                      </div>
                    </div>
                    ...
                    <DataTable
                      ...
                      loadingMessage={t("loadingMcpServers")}
                      noDataMessage={
                        <PublicHubEmptyState title={t("noMatchingMcpServers")} body={t("publicAdjustMcpHint")} />
                      }
                      size="compact"
                    />
                    ...
                    <div className="mt-8 text-center">
                      <p className="text-sm text-muted-foreground">
                        {t("showingMcpOf", { total: mcpHubData?.length || 0, shown: filteredMcpData.length })}
                      </p>
                    </div>
                  </TabsContent>
                )}
            ...
            <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[1000px]">
              <DialogHeader>
                <DialogTitle className="flex min-w-0 items-center space-x-2">
                  <span className="break-words">{selectedModel?.model_group || t("modelDetailsTitle")}</span>
                  {selectedModel && (
                    <Tooltip>
                      <TooltipTrigger
                        render={
                      ...
                      />
                      <TooltipContent>{t("copyModelName")}</TooltipContent>
                    </Tooltip>
                  )}
                </DialogTitle>
              </DialogHeader>
              {selectedModel && (
                <div className="space-y-6">
                  {/* Model Overview */}
                  <div>
                    <p className="text-lg font-semibold mb-4">{t("modelOverview")}</p>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <p className="font-medium">{t("modelNameLabel")}</p>
                        <p>{selectedModel.model_group}</p>
                      </div>
                      <div>
                        <p className="font-medium">{t("modeLabel")}</p>
                        <p>{selectedModel.mode || t("notSpecified")}</p>
                      </div>
                      <div>
                        <p className="font-medium">{t("providersLabel")}</p>
                    ...
                    {selectedModel.model_group.includes("*") && (
                      <div className="bg-info/10 border border-info/20 rounded-lg p-4 mb-4">
                        <div className="flex items-start space-x-2">
                          ...
                          <div>
                            <p className="font-medium text-info mb-2">{t("wildcardRoutingTitle")}</p>
                            <p className="text-sm text-info mb-2">
                              {t.rich("wildcardRoutingBody", {
                                code: (chunks) => (
                                  <code className="bg-info/15 px-1 py-0.5 rounded-sm text-xs">{chunks}</code>
                                ),
                              })}
                            </p>
                            <p className="text-sm text-info">
                              {t.rich("wildcardRoutingExample", {
                                model: selectedModel.model_group,
                                exampleValue: selectedModel.model_group.replaceAll("*", "my-custom-value"),
                                code: (chunks) => (
                                  <code className="bg-info/15 px-1 py-0.5 rounded-sm text-xs">{chunks}</code>
                                ),
                                example: (chunks) => (
                  ...
                  <div>
                    <p className="text-lg font-semibold mb-4">{t("tokenCostInfo")}</p>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="font-medium">{t("maxInputTokensLabel")}</p>
                        <p>{selectedModel.max_input_tokens?.toLocaleString() || t("notSpecified")}</p>
                      </div>
                      <div>
                        <p className="font-medium">{t("maxOutputTokensLabel")}</p>
                        <p>{selectedModel.max_output_tokens?.toLocaleString() || t("notSpecified")}</p>
                      </div>
                      <div>
                        <p className="font-medium">{t("inputCostPer1MLabel")}</p>
                        <p>
                      ...
                      <div>
                        <p className="font-medium">{t("outputCostPer1MLabel")}</p>
                        <p>
                  ...
                  {/* Capabilities */}
                  <div>
                    <p className="text-lg font-semibold mb-4">{t("capabilitiesTitle")}</p>
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        const capabilities = getModelCapabilities(selectedModel);

                        if (capabilities.length === 0) {
                          return <p className="text-muted-foreground">{t("noSpecialCapabilities")}</p>;
                        }
                  ...
                  {(selectedModel.tpm || selectedModel.rpm) && (
                    <div>
                      <p className="text-lg font-semibold mb-4">{t("rateLimitsTitle")}</p>
                      <div className="grid grid-cols-2 gap-4">
                        {selectedModel.tpm && (
                          <div>
                            <p className="font-medium">{t("tokensPerMinuteLabel")}</p>
                            <p>{selectedModel.tpm.toLocaleString()}</p>
                          </div>
                        )}
                        {selectedModel.rpm && (
                          <div>
                            <p className="font-medium">{t("requestsPerMinuteLabel")}</p>
                  ...
                  {selectedModel.supported_openai_params && selectedModel.supported_openai_params.length > 0 && (
                    <div>
                      <p className="text-lg font-semibold mb-4">{t("supportedOpenaiParamsTitle")}</p>
                      <div className="flex flex-wrap gap-2">
                  ...
                  {/* Usage Example */}
                  <div>
                    <p className="text-lg font-semibold mb-4">{t("usageExampleTitle")}</p>
          ...
          <Dialog open={isAgentModalVisible} onOpenChange={(open) => !open && handleAgentModalCancel()}>
            <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[1000px]">
              <DialogHeader>
                <DialogTitle className="flex min-w-0 items-center space-x-2">
                  <span className="break-words">{selectedAgent?.name || t("agentDetailsTitle")}</span>
                  {selectedAgent && (
                    <Tooltip>
                      <TooltipTrigger
                        render={
                      ...
                      />
                      <TooltipContent>{t("copyAgentName")}</TooltipContent>
                    </Tooltip>
                  )}
                </DialogTitle>
              </DialogHeader>
              {selectedAgent && (
                <div className="space-y-6">
                  {/* Agent Overview */}
                  <div>
                    <p className="text-lg font-semibold mb-4">{t("agentOverview")}</p>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                      ...
                      <div>
                        <p className="font-medium">{t("versionLabel")}</p>
                        <p>{selectedAgent.version}</p>
                      </div>
                      <div className="col-span-2">
                        <p className="font-medium">{t("descriptionLabel")}</p>
                        <p>{selectedAgent.description}</p>
                      </div>
                      {selectedAgent.url && (
                        <div>
                          <p className="font-medium">{t("urlLabel")}</p>
                          <a
                  ...
                  {selectedAgent.skills && selectedAgent.skills.length > 0 && (
                    <div>
                      <p className="text-lg font-semibold mb-4">{t("skillsTitle")}</p>
                  ...
                  {/* Input/Output Modes */}
                  <div>
                    <p className="text-lg font-semibold mb-4">{t("ioModesTitle")}</p>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="font-medium">{t("inputModesLabel")}</p>
                        <div className="flex flex-wrap gap-1 mt-1">
                      ...
                      <div>
                        <p className="font-medium">{t("outputModesLabel")}</p>
                        <div className="flex flex-wrap gap-1 mt-1">
                  ...
                  {selectedAgent.documentationUrl && (
                    <div>
                      <p className="text-lg font-semibold mb-4">{t("documentationTitle")}</p>
                      <a
                  ...
                  {/* A2A Usage Example */}
                  <div>
                    <p className="text-lg font-semibold mb-4">{t("usageA2aTitle")}</p>

                    {/* Step 1: Retrieve Agent Card */}
                    <div className="mb-4">
                      <p className="text-sm font-medium mb-2 text-foreground">{t("stepRetrieveAgentCard")}</p>
                      <div className="bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
    ...
    try:
        ...
        logger.warning(
            ...
            exc_info=True,
                    ...
                    <div>
                      <p className="text-sm font-medium mb-2 text-foreground">{t("stepCallAgent")}</p>
                      <div className="bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
                        <pre className="text-xs">
                          {`client = A2AClient(
...
print(response.model_dump(mode='json', exclude_none=True))`;
          ...
          <Dialog open={isMcpModalVisible} onOpenChange={(open) => !open && handleMcpModalCancel()}>
            <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[1000px]">
              <DialogHeader>
                <DialogTitle className="flex min-w-0 items-center space-x-2">
                  <span className="break-words">{selectedMcpServer?.server_name || t("mcpDetailsTitle")}</span>
                  {selectedMcpServer && (
                    <Tooltip>
                      <TooltipTrigger
                        render={
                      ...
                      />
                      <TooltipContent>{t("copyServerName")}</TooltipContent>
                    </Tooltip>
                  )}
                </DialogTitle>
              </DialogHeader>
              {selectedMcpServer && (
                <div className="space-y-6">
                  {/* Server Overview */}
                  <div>
                    <p className="text-lg font-semibold mb-4">{t("serverOverview")}</p>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <p className="font-medium">{t("serverNameLabel")}</p>
                        <p>{selectedMcpServer.server_name}</p>
                      </div>
                      <div>
                        <p className="font-medium">{t("transportLabel")}</p>
                        <Badge variant="secondary">{selectedMcpServer.transport}</Badge>
                      </div>
                      {selectedMcpServer.alias && (
                        <div>
                          <p className="font-medium">{t("aliasLabel")}</p>
                          <p>{selectedMcpServer.alias}</p>
                        </div>
                      )}
                      <div>
                        <p className="font-medium">{t("authTypeLabel")}</p>
                  ...
                  {selectedMcpServer.mcp_info && Object.keys(selectedMcpServer.mcp_info).length > 0 && (
                    <div>
                      <p className="text-lg font-semibold mb-4">{t("additionalInfoTitle")}</p>
