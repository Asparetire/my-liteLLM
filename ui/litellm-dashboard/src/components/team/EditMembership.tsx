...
const buildMemberSchema = (
  ...
  const shape = {
    user_email: z.string().refine(isEmailish, invalidEmailMessage).nullish(),
...
const MemberModal = <T extends BaseMember>({
  ...
  const schema = useMemo(() => buildMemberSchema(config, t("invalidEmail"), t("roleRequired")), [config, t]);
          ...
          case "numerical":
            return (
              <NumericalInput
                ...
                min={field.min || 0}
                style={{ width: "100%" }}
                placeholder={field.placeholder || t("enterNumber")}
                value={value ?? ""}
          ...
          case "multi-select":
            return (
              <MultiSelect
                ...
                placeholder={field.placeholder || t("selectOptions")}
              />
            );
  ...
  return (
    <Dialog open={visible} onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[1000px]">
        <DialogHeader>
          <DialogTitle>{config.title || (mode === "add" ? t("addMember") : t("editMember"))}</DialogTitle>
        </DialogHeader>
        <form onSubmit={form.handleSubmit(handleSubmit)}>
          <FieldGroup>
            {config.showEmail && (
              <FormField control={form.control} name="user_email" label={t("emailLabel")}>
            ...
            {config.showUserId && (
              <FormField control={form.control} name="user_id" label={t("userIdLabel")}>
                {({ ref, value, onChange, ...rest }) => (
                  <Input
                    {...rest}
                    ref={ref}
                ...
                <span className="flex items-center gap-2">
                  ...
                  {mode === "edit" && initialData && (
                    <span className="text-sm text-muted-foreground">
                      {t("currentRole", { role: getRoleLabel(initialData.role) })}
                    </span>
                  )}
                </span>
              }
