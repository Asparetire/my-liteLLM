...
function UserField({ userAlias, userEmail, userId }: { userAlias?: string | null; userEmail: string; userId: string }) {
  ...
  const labelEl = (
    <div className="flex items-center gap-1">
      ...
      <span className="text-xs uppercase tracking-[0.05em] text-muted-foreground">{t("user")}</span>
    </div>
  );
        ...
        <div key={label} className="flex flex-col min-w-0">
          ...
          {value ? (
            <div className="flex min-w-0 items-center gap-1">
              ...
              <CopyButton value={value} label={t("copyField", { name: label })} iconClassName="size-3.5" />
            </div>
          ) : (
    ...
    <div>
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="m-0 flex items-center gap-1 text-2xl font-semibold">
              {data.keyName}
              <CopyButton value={data.keyName} label={t("copyKeyAlias")} iconClassName="size-4" />
            </h3>
            {isBlocked && (
          ...
          <div className="flex min-w-0 items-center gap-1">
            <span className="min-w-0 break-words text-muted-foreground">{t("keyIdDisplay", { id: data.keyId })}</span>
            <CopyButton value={data.keyId} label={t("copyKeyId")} iconClassName="size-3.5" />
          </div>
        </div>
        {canModifyKey && (
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger render={<Button variant="outline" size="icon" aria-label={t("moreKeyActions")} />}>
      ...
      <div className="flex items-stretch gap-10" style={{ marginBottom: 40 }}>
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <LabeledField label={t("expires")} value={data.expires} icon={<Timer className="size-3.5" />} />
        </div>

        <Separator orientation="vertical" />

        <div className="flex min-w-0 flex-col gap-4">
          <LabeledField label={t("createdAt")} value={data.createdAt} icon={<Calendar className="size-3.5" />} />
          <LabeledField
            label={t("createdBy")}
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <LabeledField label={t("lastUpdated")} value={data.lastUpdated} icon={<Clock className="size-3.5" />} />
          <LabeledField label={t("lastActive")} value={data.lastActive} icon={<Zap className="size-3.5" />} />
        </div>
