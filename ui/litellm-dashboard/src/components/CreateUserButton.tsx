...
const EmailInvitationsNotice: React.FC = () => {
  ...
  return (
    <Alert variant="info" className="mb-4">
      <Info />
      <AlertTitle>{t("emailInvitationsTitle")}</AlertTitle>
      <AlertDescription>
        {t("emailInvitationsBody")}{" "}
        <a href="https://docs.litellm.ai/docs/proxy/email" target="_blank" rel="noreferrer">
          {t("emailInvitationsLink")}
        </a>
      </AlertDescription>
    </Alert>
  );
...
export const CreateUserButton: React.FC<CreateuserProps> = ({
  ...
  const handleCreate = async (formValues: CreateUserFormValues) => {
    try {
      ...
      toast.success(t("apiUserCreated"));
      form.reset(defaultValues);
      localStorage.removeItem("userData" + userID);
    } catch (error: any) {
      const errorMessage = error.response?.data?.detail || error?.message || t("createUserFailed");
  ...
  const userEmailField = (
    <FormField control={form.control} name="user_email" label={t("labelUserEmail")}>
      {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} />}
    </FormField>
  );

  const teamField = (
    <FormField control={form.control} name="team_id" label={t("labelTeam")} description={t("teamFieldDesc")}>
      {({ id, value, onChange }) => <TeamDropdown id={id} value={value} onChange={onChange} />}
    </FormField>
  );

  const metadataField = (
    <FormField control={form.control} name="metadata" label={t("labelMetadata")}>
      {({ ref, value, ...control }) => (
        <Textarea {...control} ref={ref} value={value ?? ""} rows={4} placeholder={t("metadataPlaceholder")} />
      )}
    </FormField>
  );
    ...
    return (
      <TooltipProvider>
        <form onSubmit={form.handleSubmit(handleCreate)}>
          ...
          <FieldGroup>
            {userEmailField}
            {roleField(t("userRoleLabel"))}
            {teamField}
            {metadataField}
            {sendInviteEmailField}
          </FieldGroup>
          <div className="mt-4 text-right">
            <Button type="submit">{t("createUserBtn")}</Button>
          </div>
        </form>
      </TooltipProvider>
    );
      ...
      <Dialog open={isModalVisible} onOpenChange={(open) => !open && handleCancel()}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
          ...
          <div className="flex flex-col gap-3">
            <p className="mb-1 text-sm text-foreground">{t("createUserHint")}</p>
            <EmailInvitationsNotice />
          </div>
          <TooltipProvider>
            <form onSubmit={form.handleSubmit(handleCreate)}>
              <FieldGroup>
                {userEmailField}
                {roleField(labelWithHint(t("labelGlobalRole"), t("globalProxyRoleHint")))}
                {teamField}
                ...
                <Collapsible open={isPersonalKeyOpen} onOpenChange={setIsPersonalKeyOpen}>
                  <CollapsibleTrigger className="flex w-full items-center gap-2 rounded-md border border-border px-3 py-2 text-left text-sm font-semibold text-foreground">
                    <ChevronRight
                      ...
                      aria-hidden
                    />
                    {t("personalKeyCreation")}
                  </CollapsibleTrigger>
                  <CollapsibleContent className="pt-4">
                    <FormField
                      control={form.control}
                      name="models"
                      label={labelWithHint(t("modelsLabel"), t("modelsOutsideTeamHint"))}
                      description={t("modelsOutsideTeamHint")}
                    >
                      {({ value, onChange }) => (
                        <MultiSelect
                          options={[
