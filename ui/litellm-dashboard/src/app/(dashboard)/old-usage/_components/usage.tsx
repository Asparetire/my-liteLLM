...
const UsagePage: React.FC<UsagePageProps> = ({ accessToken, token, userRole, userID, keys, premiumUser }) => {
  ...
  const tagOptions: TagOption[] = [
    { value: ALL_TAGS, label: t("allTagsOption"), disabled: false },
    ...allTagNames
      .filter((tag) => tag !== ALL_TAGS)
      .map((tag) => ({
        value: tag,
        label: premiumUser ? tag : t("enterpriseTagLabel", { tag }),
    ...
    return (
      <div className="w-full p-8">
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">{t("proxyWideAdminOnly")}</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  if (proxySettings?.DISABLE_EXPENSIVE_DB_QUERIES) {
    return (
      <div className="w-full p-8">
        <Card>
          <CardHeader>
            <CardTitle>{t("dbQueryLimitTitle")}</CardTitle>
          </CardHeader>
    ...
    <div className="w-full p-8">
      <Tabs defaultValue="all-up">
        <TabsList variant="line" className="mt-2">
          <TabsTrigger value="all-up">{t("tabAllUp")}</TabsTrigger>

          {isAdminOrAdminViewer(userRole) && (
            <>
              <TabsTrigger value="team-based-usage">{t("tabTeamBased")}</TabsTrigger>
              <TabsTrigger value="customer-usage">{t("tabCustomer")}</TabsTrigger>
              <TabsTrigger value="tag-based-usage">{t("tabTagBased")}</TabsTrigger>
            </>
          )}
        </TabsList>
              ...
              <div className="grid h-screen w-full grid-cols-2 gap-2">
                <div className="col-span-2">
                  <p className="mt-2 mb-2 text-lg text-muted-foreground">
                    {t("projectSpendRange", {
                      ...
                      endDay: new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate(),
                    })}
                  </p>
                ...
                <div className="col-span-1">
                  <Card className="h-full">
                    <CardHeader>
                      <CardTitle>{t("topVirtualKeys")}</CardTitle>
                    </CardHeader>
                    <CardContent>
                          ...
                          <Table>
                            <TableHeader>
                              <TableRow>
                                ...
                                <TableHead>{t("colSpend")}</TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                    ...
                    <div className="grid grid-cols-2">
                      <div>
                        <p className="text-[15px] font-normal text-muted-foreground">
                          {t("apiRequestsLabel", { count: valueFormatterNumbers(globalActivity.sum_api_requests) })}
                        </p>
                        <AreaChart
                          className="h-40"
                      ...
                      <div>
                        <p className="text-[15px] font-normal text-muted-foreground">
                          {t("tokensLabel", { count: valueFormatterNumbers(globalActivity.sum_total_tokens) })}
                        </p>
                        <BarChart
                          className="h-40"
                          data={globalActivity.daily_data}
        ...
        <TabsContent value="team-based-usage" keepMounted>
          <div className="grid h-[75vh] w-full grid-cols-2 gap-2">
            <div className="col-span-2">
              <Card className="mb-2">
                <CardHeader>
                  <CardTitle>{t("totalSpendPerTeam")}</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>{t("dailySpendPerTeam")}</CardTitle>
                </CardHeader>
                <CardContent>
                  <BarChart
                    className="h-72"
        ...
        <TabsContent value="customer-usage" keepMounted>
          ...
          <div className="grid grid-cols-2">
            ...
            <div>
              <p className="text-sm text-muted-foreground">{t("selectKeyLabel")}</p>
              <Select
              ...
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={t("allKeys")}>
                    {(token: string | null) => selectableKeys.find((key) => key.token === token)?.alias ?? t("allKeys")}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value={null}>{t("allKeys")}</SelectItem>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>{t("colSpend")}</TableHead>
                      <TableHead>{t("colTotalEvents")}</TableHead>
                    </TableRow>
                  </TableHeader>
        ...
        <TabsContent value="tag-based-usage" keepMounted>
            ...
            <div>
              ...
              >
                <ComboboxChips render={<div ref={anchor} />}>
                  ...
                  <ComboboxChipsInput placeholder={t("selectTagsPlaceholder")} />
                </ComboboxChips>
          ...
          <div className="mb-4 grid h-[75vh] w-full grid-cols-2 gap-2">
            <div className="col-span-2">
              <Card>
                <CardHeader>
                  <CardTitle>{t("spendPerTag")}</CardTitle>
                </CardHeader>
                <CardContent className="flex flex-col gap-2">
                  <p className="text-sm text-muted-foreground">
                    {t.rich("tagTrackingHint", {
                      docsLink: (chunks) => (
                        <a
                          className="text-primary"
