...
const TAB_LABEL_KEYS: Record<ModelTabSlug, string> = {
  add: "tabAddModel",
  "auto-routers": "tabAutoRouters",
  "llm-credentials": "tabLlmCredentials",
  "pass-through": "tabPassThroughEndpoints",
  health: "tabHealthStatus",
  "retry-settings": "tabRetrySettings",
  "model-group-alias": "tabModelGroupAlias",
  "access-group-budgets": "tabAccessGroupBudgets",
  "price-data": "tabPriceDataReload",
};
...
export default function ModelsAndEndpointsPage() {
  ...
  const allModelsLabel = isAdmin ? t("allModels") : t("yourModels");
    ...
    <div className="mx-4">
      <div className="mt-2 flex w-full flex-col gap-2 p-8">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold">{t("modelManagementTitle")}</h2>
            {isAdmin ? (
              <p className="text-sm text-muted-foreground">{t("adminSubtitle")}</p>
            ) : (
              <p className="text-sm text-muted-foreground">{t("teamAdminSubtitle")}</p>
            )}
          </div>
        </div>
          ...
          <Tabs value={activeKey} onValueChange={setActiveKey}>
            <div className="flex min-w-0 flex-nowrap items-center gap-3 border-b">
              ...
              <div className="flex shrink-0 items-center gap-2 pb-1">
                {lastRefreshed && (
                  <span className="text-xs text-muted-foreground">{t("lastRefreshed", { time: lastRefreshed })}</span>
                )}
                <Button variant="ghost" size="icon-sm" onClick={handleRefreshClick} aria-label={t("refreshModelsAria")}>
                  <RefreshCw />
                </Button>
              </div>
            </div>
