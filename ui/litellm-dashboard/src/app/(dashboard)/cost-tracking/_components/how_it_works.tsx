...
const HowItWorks: React.FC = () => {
  ...
  return (
    <div className="space-y-4 pt-2">
      <div>
        <h3 className="mb-1 text-sm font-medium text-foreground">{t("costCalculationTitle")}</h3>
        <p className="text-xs text-muted-foreground">
          {t("discountsAppliedHint")}{" "}
      ...
      <div>
        <h3 className="mb-1 text-sm font-medium text-foreground">{t("exampleTitle")}</h3>
        <p className="text-xs text-muted-foreground">{t("exampleBody")}</p>
      </div>
      <div>
        <h3 className="mb-1 text-sm font-medium text-foreground">{t("validRangeTitle")}</h3>
        <p className="text-xs text-muted-foreground">{t("validRangeBody")}</p>
      </div>

      <div className="border-t border-border pt-4">
        <h3 className="mb-2 text-sm font-medium text-foreground">{t("validatingDiscountsTitle")}</h3>
        <p className="mb-3 text-xs text-muted-foreground">{t("validatingDiscountsHint")}</p>
        <CodeBlock
          language="bash"
  ...
  -d '{
    ...
    "messages": [{"role": "user", "content": "Hello"}]
        ...
        />
        <p className="mb-2 mt-3 text-xs text-muted-foreground">{t("lookForHeadersHint")}</p>
        <div className="space-y-1.5">
          <div className="flex items-start gap-3">
            <code className="whitespace-nowrap rounded-sm bg-muted px-2 py-1 font-mono text-xs text-foreground">
              x-litellm-response-cost
            </code>
            <p className="text-xs text-muted-foreground">{t("finalCostAfterDiscount")}</p>
          </div>
          <div className="flex items-start gap-3">
            <code className="whitespace-nowrap rounded-sm bg-muted px-2 py-1 font-mono text-xs text-foreground">
              x-litellm-response-cost-original
            </code>
            <p className="text-xs text-muted-foreground">{t("originalCostBeforeDiscount")}</p>
          </div>
          <div className="flex items-start gap-3">
            <code className="whitespace-nowrap rounded-sm bg-muted px-2 py-1 font-mono text-xs text-foreground">
              x-litellm-response-cost-discount-amount
            </code>
            <p className="text-xs text-muted-foreground">{t("amountDiscounted")}</p>
          </div>
        </div>
      </div>

      <div className="border-t border-border pt-4">
        <h3 className="mb-3 text-sm font-medium text-foreground">{t("discountCalculatorTitle")}</h3>
        <p className="mb-3 text-xs text-muted-foreground">{t("enterHeaderValuesHint")}</p>
        ...
        {calculatedDiscount && (
          <div className="rounded-lg border border-border bg-muted/50 p-4">
            <p className="mb-2 text-sm font-medium text-foreground">{t("calculatedResultsLabel")}</p>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-xs text-muted-foreground">{t("originalCostLabel")}</p>
                <code className="font-mono text-xs text-foreground">${calculatedDiscount.originalCost}</code>
              </div>
              <div className="flex items-center justify-between">
                <p className="text-xs text-muted-foreground">{t("finalCostLabel")}</p>
                <code className="font-mono text-xs text-foreground">${calculatedDiscount.finalCost}</code>
              </div>
              <div className="flex items-center justify-between">
                <p className="text-xs text-muted-foreground">{t("discountAmountResultLabel")}</p>
                <code className="font-mono text-xs text-foreground">${calculatedDiscount.discountAmount}</code>
              </div>
              <div className="flex items-center justify-between border-t border-border pt-2">
                <p className="text-xs font-semibold text-foreground">{t("discountAppliedLabel")}</p>
