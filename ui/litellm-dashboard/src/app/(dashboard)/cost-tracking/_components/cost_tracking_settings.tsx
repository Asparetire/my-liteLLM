...
const DOCS_LINKS = [
  { labelKey: "docCustomPricing", href: "https://docs.litellm.ai/docs/proxy/custom_pricing" },
  { labelKey: "docSpendTracking", href: "https://docs.litellm.ai/docs/proxy/cost_tracking" },
] as const;

type RemovalKind = "discount" | "margin";
    ...
    <div className="w-full p-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2">
            <p className="text-xl font-medium text-foreground">{t("settingsTitle")}</p>
      ...
      <div className="bg-card rounded-lg shadow-sm w-full max-w-full space-y-4">
        ...
        {isProxyAdmin && (
          <Collapsible className="rounded-lg border">
            <SectionHeader title={t("sectionProviderDiscounts")} description={t("sectionProviderDiscountsDesc")} />
                ...
                <TabsContent value="discounts" keepMounted>
                  <div className="p-6">
                    <div className="flex justify-end mb-4">
                      <Button onClick={() => setIsModalVisible(true)}>{t("addProviderDiscountBtnPlus")}</Button>
                    </div>
                    {isFetching ? (
                      ...
                      <div className="py-16 px-6 text-center">
                        ...
                        <p className="text-foreground font-medium mb-2">{t("noProviderDiscounts")}</p>
                        <p className="text-muted-foreground text-sm">{t("emptyDiscountsHint")}</p>
                      </div>
                    )}
                  </div>
                </TabsContent>
        ...
        {isProxyAdmin && (
          <Collapsible className="rounded-lg border">
            <SectionHeader title={t("sectionFeeMargin")} description={t("sectionFeeMarginDesc")} />
            <CollapsibleContent className="px-0">
              <div className="p-6">
                <div className="flex justify-end mb-4">
                  <Button onClick={() => setIsMarginModalVisible(true)}>{t("addProviderMarginBtnPlus")}</Button>
                </div>
                {isFetching ? (
                  <div className="py-12 text-center">
                    <p className="text-muted-foreground">{t("loadingConfig")}</p>
                  ...
                  <div className="py-16 px-6 text-center">
                    ...
                    <p className="text-foreground font-medium mb-2">{t("noProviderMargins")}</p>
                    <p className="text-muted-foreground text-sm">{t("emptyMarginsHint")}</p>
                  </div>
                )}
              </div>
            </CollapsibleContent>
        ...
        {isProxyAdmin && (
          <Collapsible className="rounded-lg border">
            <SectionHeader title={t("sectionBlockUnpriced")} description={t("sectionBlockUnpricedDesc")} />
            <CollapsibleContent className="px-0">
              <div className="p-6">
                <div className="flex items-center justify-between">
                  <div className="pr-6">
                    <p className="text-foreground font-medium">{t("blockUnpricedLabel")}</p>
                    <p className="text-sm text-muted-foreground mt-1">{t("blockUnpricedDesc")}</p>
                  </div>
                  <Switch
        ...
        <Collapsible defaultOpen={true} className="rounded-lg border">
          <SectionHeader title={t("sectionPricingCalculator")} description={t("sectionPricingCalculatorDesc")} />
          <CollapsibleContent className="px-0">
            <div className="p-6">
      ...
      {pendingRemoval && (
        <AlertDialog open onOpenChange={(open) => !open && !isRemoving && setPendingRemoval(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                {pendingRemoval.kind === "discount" ? t("removeDiscountTitle") : t("removeMarginTitle")}
              </AlertDialogTitle>
              <AlertDialogDescription>
                {pendingRemoval.kind === "discount"
                  ? t("removalDiscountConfirm", { name: pendingRemoval.displayName })
                  : t("removalMarginConfirm", { name: pendingRemoval.displayName })}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel disabled={isRemoving}>{t("cancelBtn")}</AlertDialogCancel>
              <Button variant="destructive" onClick={handleConfirmRemoval} disabled={isRemoving}>
                {isRemoving ? t("removingBtn") : t("removeBtn")}
              </Button>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
        ...
        <DialogContent className="top-8 max-h-[calc(100dvh-4rem)] translate-y-0 overflow-y-auto sm:max-w-[1000px]">
          <DialogHeader>
            <div className="flex items-center space-x-3 pb-4 border-b border-border">
              <DialogTitle className="text-xl font-semibold text-foreground">{t("addProviderDiscountBtn")}</DialogTitle>
            </div>
          </DialogHeader>
          <div className="mt-6">
            <p className="text-sm text-muted-foreground mb-6">{t("addDiscountDialogHint")}</p>
      ...
      <Dialog open={isMarginModalVisible} onOpenChange={(open) => !open && handleMarginModalCancel()}>
        <DialogContent className="top-8 max-h-[calc(100dvh-4rem)] translate-y-0 overflow-y-auto sm:max-w-[1000px]">
          <DialogHeader>
            <div className="flex items-center space-x-3 pb-4 border-b border-border">
              <DialogTitle className="text-xl font-semibold text-foreground">{t("addProviderMarginBtn")}</DialogTitle>
            </div>
          </DialogHeader>
          <div className="mt-6">
            <p className="text-sm text-muted-foreground mb-6">{t("addMarginDialogHint")}</p>
            <form onSubmit={(event) => event.preventDefault()} className="space-y-6">
              <AddMarginForm
                marginConfig={marginConfig}
                selectedProvider={selectedMarginProvider}
