...
export function UserEditView({
  ...
  const schema = useMemo(() => budgetSchema(unlimitedBudget, t("budgetOrUnlimitedError")), [unlimitedBudget, t]);
  ...
  return (
    <TooltipProvider>
      <form onSubmit={form.handleSubmit(handleSubmit)}>
        <FieldGroup>
          {!isBulkEdit && (
            <FormField control={form.control} name="user_id" label={t("labelUserId")}>
              {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} disabled />}
            </FormField>
          )}

          {!isBulkEdit && (
            <FormField control={form.control} name="user_email" label={t("labelEmail")}>
              {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} />}
            </FormField>
          )}

          <FormField control={form.control} name="user_alias" label={t("labelUserAlias")}>
            {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} />}
          </FormField>
          ...
          <FormField
            ...
            name="user_role"
            label={labelWithHint(t("labelGlobalRole"), t("globalRoleHint"))}
          >
            {({ id, value, onChange }) => (
              <Select
                items={roleOptions}
          ...
          <FormField control={form.control} name="budget_duration" label={t("labelBudgetReset")}>
          ...
          {canEditMcpPermissions && (
            <>
              <FormField
                ...
                label={labelWithHint(t("labelMcpServers"), t("mcpServersHint"))}
              >
                {({ value, onChange }) => (
                  <MCPServerSelector
                    onChange={onChange}
