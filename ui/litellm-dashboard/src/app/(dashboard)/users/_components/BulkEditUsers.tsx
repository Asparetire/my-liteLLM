  ...
  const handleSubmit = async (formValues: any) => {
    ...
    try {
      ...
      if (!hasUserUpdates && !hasTeamAdditions) {
        toast.fromError(t("modifyAtLeastOne"));
        return;
      }
      ...
      if (hasUserUpdates) {
        if (updateAllUsers) {
          ...
          successMessages.push(t("updatedAllMessage", { count: result.total_requested }));
        } else {
          await userBulkUpdateUserCall(accessToken, updatePayload, userIds);
          successMessages.push(t("updatedCountMessage", { count: userIds.length }));
        }
      }
      ...
      if (hasTeamAdditions) {
        ...
        if (successfulTeams.length > 0) {
          ...
          successMessages.push(
            t("addedToTeamsMessage", { teams: successfulTeams.length, count: totalAdditions }),
          );
        }

        if (failedTeams.length > 0) {
          toast.warning(t("addTeamsFailed", { count: failedTeams.length }));
        }
      }
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        <DialogHeader>
          <DialogTitle>
            {updateAllUsers ? t("bulkEditTitleAll") : t("bulkEditTitleCount", { count: selectedUsers.length })}
          </DialogTitle>
        </DialogHeader>
        {allowAllUsers && (
          <div className="mb-4">
            ...
            {updateAllUsers && (
              <div className="mt-2">
                <span className="text-xs text-warning">{t("updateAllWarning")}</span>
              </div>
            )}
          </div>
        )}
        ...
        {!updateAllUsers && (
          <div className="mb-4">
            <h5 className="mb-2 text-sm font-semibold text-foreground">
              {t("selectedUsersTitle", { count: selectedUsers.length })}
            </h5>
            <div className="max-h-[200px] overflow-y-auto rounded-md border border-border">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[30%]">{t("labelUserId")}</TableHead>
                    <TableHead className="w-[25%]">{t("labelEmail")}</TableHead>
                    <TableHead className="w-[25%]">{t("colCurrentRole")}</TableHead>
                    <TableHead className="w-[20%]">{t("colBudgetPlain")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
        ...
        <div className="mb-4">
          <p className="text-sm text-foreground">
            <strong>{t("instructionsTitle")}</strong> {t("instructionsBody")}
          </p>
        </div>
        ...
        <Card size="sm" className="mb-4 bg-muted/50">
          <CardHeader>
            <CardTitle>{t("teamManagementTitle")}</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <>
                  ...
                  <div>
                    ...
                    <NumericalInput
                      id={teamBudgetId}
                      className="mt-2"
                      placeholder={t("teamBudgetPlaceholder")}
                      value={teamBudget ?? ""}
                      ...
                      step={0.01}
                    />
                    <span className="text-xs text-muted-foreground">{t("teamBudgetHint")}</span>
                  </div>

                  <span className="text-xs text-muted-foreground">{t("defaultRoleNote")}</span>
                </>
              )}
            </div>
          </CardContent>
        ...
        {loading && (
          <div className="mt-2.5 text-center">
            <span className="text-sm text-foreground">
              {updateAllUsers ? t("updatingAllUsers") : t("updatingCountUsers", { count: selectedUsers.length })}
            </span>
          </div>
        )}
      </DialogContent>
