...
describe("SpendByProvider", () => {
  ...
  it("should display provider data in table", () => {
    ...
    expect(screen.getByText("201 tokens")).toBeInTheDocument();
  });

  it("should display spend values as whole token counts", () => {
    render(<SpendByProvider loading={false} isDateChanging={false} providerSpend={mockProviderSpend} />);
    expect(screen.getByText("151 tokens")).toBeInTheDocument();
    expect(screen.getByText("201 tokens")).toBeInTheDocument();
  });
  ...
  it("should handle provider with null provider name", () => {
    ...
    expect(screen.getAllByText("100 tokens").length).toBeGreaterThan(0);
  });
  ...
  it("renders one cyan donut sector per visible provider with the token total as center label", () => {
    ...
    expect(centerLabels).toContain("351 tokens");
  });
  ...
  it("adds the unknown provider slice and updates the center total when Show Unknown is on", () => {
    ...
    expect(container.querySelector("text.fill-foreground")?.textContent).toBe("151 tokens");

    fireEvent.click(screen.getAllByRole("switch")[1]);

    expect(container.querySelectorAll(".recharts-pie-sector path")).toHaveLength(2);
    expect(container.querySelector("text.fill-foreground")?.textContent).toBe("201 tokens");
  });
